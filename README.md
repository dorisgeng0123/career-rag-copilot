...
1. 用户上传截图或粘贴 JD 后，前端自动触发后台解析，不再要求用户额外点击“解析”按钮。
2. 如果是截图，先调用 OCR / 视觉模型提取文字。
3. 将 JD 文本送入模型解析。
4. 抽取公司、岗位、职级、职责、能力标签和关键词。
5. 生成结构化 `JDContext`，并刷新到页面卡片中。
6. 如果用户正在修改 JD，旧解析结果不会覆盖新输入。
...
页面会用进度条提示当前处于“等待 JD 输入 / 后台解析中 / 解析完成”哪个阶段。生成回答前还会再次校验当前 JD 是否已经解析完成，避免使用空 JD 或上一份 JD 生成回答。

这个步骤的作用是把非结构化 JD 转成后续问题推荐、chunk 检索和回答生成都能使用的岗位上下文；对用户来说它是后台隐藏步骤，对系统来说仍然是关键数据准备层。
...
为了避免页面空白，JD 后台解析完成后会先展示一组本地规则生成的默认推荐问题；用户点击“生成推荐问题”时，才会按当前 JD 和任务模式请求模型生成更贴近岗位的增强版问题。
